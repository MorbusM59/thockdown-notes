...
const oldUndoRedoStart = code.indexOf("  const handleUndo = useCallback(() => {");
const secondEndStr = "  }, [applySnapshotProgrammatically, content, replaceEditHistory]);\n";
const firstEnd = code.indexOf(secondEndStr, oldUndoRedoStart);
const secondEnd = code.indexOf(secondEndStr, firstEnd + secondEndStr.length) + secondEndStr.length;

if (oldUndoRedoStart > -1 && secondEnd > -1) {
...
`;
  code = code.substring(0, oldUndoRedoStart) + newUndoRedo + code.substring(secondEnd);
}

fs.writeFileSync('src/components/MarkdownEditor.tsx', code);
console.log('step2c done');
