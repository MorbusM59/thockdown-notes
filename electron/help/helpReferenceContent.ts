...
- Click response: left-click tightens the orbit (smaller radius, faster spin) and right-click widens it (larger radius, slower spin), up to 200% and down to 50% of your base radius/spin. This is a temporary pulse, not a permanent change — a quick click rises to a peak then eases back down to your base radius/spin on its own; holding the button keeps it pinned at its loudest for as long as you hold it (like holding Page Up/Down), then releasing eases it back down to baseline instead of snapping. Six more sliders tune the feel, using the same bell-curve model as Scrolling Behavior's ramp/shape/speed/max-speed: click ramp (linear → bouncy), click shape (apex bias), click speed (how long one pulse takes), click max speed (how strong the effect gets if you hold), click step (how strong a single un-held tap's pulse peaks at), and click balance (center = radius and spin both affected equally; left = radius only; right = spin only).
