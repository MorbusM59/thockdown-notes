...
- The sitemap button next to the chapter-creation button (bottom toolbar) adds an auto-generated **Table of Contents** chapter, pinned first in the bar — before every real chapter, not draggable, and never a promotion target. It lists every heading across the parent and all of its chapters, in order, each one a working link straight to it. It's regenerated fresh every time you open it, so it's always accurate without costing anything while you're not looking at it — meaning it's also read-only (anything you tried to type would just be overwritten on your next visit) and opens straight into render view, since the links it lists aren't clickable as raw markdown text. Click the sitemap button again to remove it — it's fully disposable, regenerated from scratch any time you want it back, so there's no confirmation and no Trash for it.
