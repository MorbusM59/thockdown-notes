...
- **Noise layers** are noise whose level rises and falls in a steady cycle. They control **volume**, **modulation amplitude** (how far the level swings), **period** (seconds for one full rise and fall), **ramp**, **shape**, noise type and filtering. **Ramp** sets the curve of the swing: a sine in the middle; to the left it lingers loud and dips only briefly, to the right it stays quiet and swells only briefly. **Shape** sets where in the cycle the peak falls, so the rise and the fall can take different lengths.
