...
- Choose one of six soundscapes from the first row, then choose a channel to show its controls. The twelve channels come in five groups, each marked by its own icon and numbered within the group: two **brown noise** (waves; deep and slow, like surf or distant rumble), two **pink noise** (wind; balanced, like wind or a steady wash), two **white noise** (fan; bright, like hiss or moving air), three **rain** (cloud) and three **thunder** (lightning bolt), the rain and thunder initially spread across the stereo field. A channel's group decides its noise type. Click a channel to view it; hold left-click on a disabled channel to enable it, or hold right-click on an enabled channel to disable it. A short right-click solos a channel and shows its controls; right-click it again to clear solo, or right-click another channel to switch solo. The soloed button is highlighted. Disabled channels keep their settings, and their controls remain visible but cannot be changed. While hovering over an enabled channel button, scroll the mouse wheel to adjust its volume in 5% steps.
- **Noise layers** are noise whose level rises and falls in a steady cycle. They control **volume**, **modulation amplitude** (how far the level swings), **period** (seconds for one full rise and fall), **ramp**, **shape**, **movement**, **distance**, **width** and **tone**. **Ramp** sets the curve of the swing: a sine in the middle; to the left it lingers loud and dips only briefly, to the right it stays quiet and swells only briefly. **Shape** sets where in the cycle the peak falls, so the rise and the fall can take different lengths. **Movement** makes each cycle a little different from the last — longer or shorter, stronger or weaker — and sways the layer between left and right, sweeping past the centre as it swells. **Distance** works as it does on rain layers: near and dry, or far, dark and reverberant. **Width** runs from a single point to all around you; a narrow layer is where movement's sway can really be heard travelling. **Tone** darkens the layer to the left of centre and brightens it to the right, keeping its loudness as it goes; toward either end it begins to resonate, so wind can whistle on the left and air can hiss on the right.
- **Rain layers** control **volume** and **surface** — what the rain falls on, from soft on the left to hard on the right; the surface is the material only. At the far left it patters softly on leaves (**forest**); in the middle it splashes on pavement and plips into puddles (**street**); at the far right it rings brightly, like hail on glass (**glass**). Everything in between blends the two neighbours — something like soil or a wooden deck toward the leaves, a tin roof or a car toward the glass. **Drops** sets how many nearby drops you hear one by one each second, and **mix** the balance between the drops and the wash, the steady hiss of rain too dense to hear drop by drop: the wash alone on the left, the drops alone on the right, both at full in the middle — drops alone sound like dripping, the wash is what makes it rain. **Drips** adds large, heavy drops from gutters, eaves and branches. **Wetness** is how much water stands on it: drops land in the water with a bright splash, a spray of finer droplets and the rising *plip* of a trapped bubble, and the water deadens the surface's own ring — a wet road is the street with wetness up. **Resonance** is how much the surface rings: dead to the left (only the impact), as it is in the middle, ringing twice as long to the right. **Distance** moves a layer from near and distinct to far and diffuse. **Pan** places it and sets its width at once: in the centre its drops fall across the whole stereo field, and panned toward a side it narrows onto that side, to a single point at the edge. Layering a near, a middle and a far layer — which the rain soundscapes do — gives the rain depth.
- **Thunder layers** rumble now and then rather than playing continuously: the first peal comes within a few seconds of enabling one. Each peal rolls in gently, builds to its loudest a while in, and fades slowly. **Often** is how much of the time it thunders, shown as rumbling to silence: at the far left it never does, in the middle it is silent as long as it rumbles, and at the far right it rumbles without a pause — **1 : 99** with a 20-second peal means 20 seconds of thunder, then 33 minutes of quiet. **Character** is how harshly the deep boom breaks up, from a smooth, slow swell to a choppy, angry growl; the rumble above it stays smooth. **Contrast** works as it does for an image: to the right, a peal's loud moments get louder and its quiet ones quieter; to the left they are drawn together and the roll flattens; in the middle it is off. **Distance** runs from near, fuller and brighter, to far, a low, soft roll. **Pan** places the storm, **spread** how much of the stereo field a peal fills: in the centre up to the whole width, narrowing onto a side as it is panned toward it, the way rain's pan does, and **length** how long it rolls. **Random** varies all of the layer's other controls again after every peal, by up to a quarter of each one's range either way, so no two peals come out alike. Two or three layers at different distances and pans make a storm moving around you.
