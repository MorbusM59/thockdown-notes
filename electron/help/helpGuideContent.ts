...
- Choose one of six soundscapes from the first row, then choose a channel to show its controls. The twelve channels come in four groups of three, each marked by its own icon and numbered 1–3 within the group: **brown noise** (waves; deep and slow, like surf or distant rumble), **pink noise** (wind; balanced, like wind or a steady wash), **white noise** (fan; bright, like hiss or moving air), and **rain** (cloud), initially positioned left, center, and right in stereo. A channel's group decides its noise type. Click a channel to view it; hold left-click on a disabled channel to enable it, or hold right-click on an enabled channel to disable it. A short right-click solos a channel and shows its controls; right-click it again to clear solo, or right-click another channel to switch solo. The soloed button is highlighted. Disabled channels keep their settings, and their controls remain visible but cannot be changed. While hovering over an enabled channel button, scroll the mouse wheel to adjust its volume in 5% steps.
