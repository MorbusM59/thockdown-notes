...
- Choose one of six soundscapes from the first row, then choose a numbered channel to show its controls. All twelve channel slots remain in place. Click a channel to view it; hold left-click on a disabled channel to enable it, or hold right-click on an enabled channel to disable it. A short right-click solos a channel; right-click it again to clear solo, or right-click another channel to switch solo. The soloed button has a red outline. Disabled channels keep their settings, and their controls remain visible but cannot be changed. While hovering over an enabled channel button, scroll the mouse wheel to adjust its volume in 5% steps. Each channel controls **volume**, **modulation amplitude**, and **modulation period**; setting modulation period to **0** selects bursts. The remaining controls set burst **ramp**, **shape**, **speed**, noise type, and average of 1–100 randomly spaced events per 10 seconds. The filter slider runs from strong low-pass through no filter at its midpoint to strong high-pass.
