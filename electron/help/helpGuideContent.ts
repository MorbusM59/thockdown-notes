...
- **Overall volume** — hover the cloud-bolt button in the music player and scroll; hold Shift to move by ten. Scrolling turns ambient sound on if it was off. The volume belongs to you rather than to a soundscape: switching soundscapes keeps it, and saving one does not store it.
- Choose one of six soundscapes from the first row, then choose a numbered channel to show its controls. Channels 1–9 are noise layers; channels 10–12 are rain layers, initially positioned left, center, and right in stereo. Click a channel to view it; hold left-click on a disabled channel to enable it, or hold right-click on an enabled channel to disable it. A short right-click solos a channel; right-click it again to clear solo, or right-click another channel to switch solo. The soloed button is highlighted. Disabled channels keep their settings, and their controls remain visible but cannot be changed. While hovering over an enabled channel button, scroll the mouse wheel to adjust its volume in 5% steps.
- **Noise layers** are noise whose level rises and falls in a steady cycle. They control **volume**, **modulation amplitude** (how far the level swings), **period** (seconds for one full rise and fall), **ramp**, **shape**, **movement**, noise type and filtering. **Ramp** sets the curve of the swing: a sine in the middle; to the left it lingers loud and dips only briefly, to the right it stays quiet and swells only briefly. **Shape** sets where in the cycle the peak falls, so the rise and the fall can take different lengths. **Movement** makes each cycle a little different from the last — longer or shorter, stronger or weaker — and sways the layer between left and right, sweeping past the centre as it swells.
