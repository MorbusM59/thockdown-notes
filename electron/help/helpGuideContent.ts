...
- Choose one of six soundscapes from the first row, then choose a numbered channel to show its controls. Channels 1–9 are noise layers; channels 10–12 are procedural rain layers, initially positioned left, center, and right in stereo. Click a channel to view it; hold left-click on a disabled channel to enable it, or hold right-click on an enabled channel to disable it. A short right-click solos a channel; right-click it again to clear solo, or right-click another channel to switch solo. The soloed button has a red outline. Disabled channels keep their settings, and their controls remain visible but cannot be changed. While hovering over an enabled channel button, scroll the mouse wheel to adjust its volume in 5% steps. Noise layers control **modulation amplitude** and **modulation period**; setting period to **0** selects bursts. Their remaining controls set burst **ramp**, **shape**, **speed**, noise type, average burst density, and filtering. Rain layers control **density** in drops per second, **distance**, **pan**, and the **bass** and **treble** levels of each impact. Distance moves from near and distinct to far and diffuse; pan moves each layer from left to right. The Rain on glass and Passing storm soundscapes use all three rain layers.
