...
- Choose one of six soundscapes from the first row, then choose a channel to show its controls below. There are eighteen channels, each marked by the icon of its kind: six **noise** (a wave), three **rain** (a cloud), three **thunder** (a lightning bolt), two **water** (a drop), two **fire** (a flame) and two **chimes** (a bell). Click a channel to view it; hold left-click on a disabled channel to enable it, or hold right-click on an enabled channel to disable it — a disabled channel costs nothing to play. A short right-click solos a channel and shows its controls; right-click it again to clear solo, or right-click another channel to switch solo. The soloed button is highlighted. Disabled channels keep their settings, and their controls remain visible but cannot be changed. While hovering over an enabled channel button, scroll the mouse wheel to adjust its volume in 5% steps.
...
- **Rain channels** are laid out in rows of three. The first row is the rain and what it falls on: **volume**, **surface** — soft to hard: **leaves**, **canvas** (a tent or an awning — a dull thump), the **street**, **tin** (a shed or a car roof) and **glass**, with everything between two of them a blend — and **ring**, how much the surface rings: dead to the left, as it is in the middle, twice as long to the right. The next four rows are its parts, each as how much, how loud (**level**: off at the far left, as it is at three quarters, louder above) and **tone**, its pitch. **Drops** are the ones heard one by one, from drizzle to downpour. The **wash** is the steady hiss of rain too dense to hear drop by drop, from a sparse patter to a smooth hiss; with its level off the drops play alone, which sounds like dripping — the wash is what makes it rain. **Drips** are large, heavy drops from gutters, eaves and branches. **Wet** is standing water: drops land in it with a bright splash, a spray of finer droplets and the rising *plip* of a trapped bubble, and its film deadens the surface's ring — a wet road is the street with wetness up; the splashes' tone moves their pitch and the bubbles' size together. Under **Place**, **width** runs from a single point at the pan to as wide as the pan allows.
...
- **Water channels** are a brook or a stream: a cloud of bubbles over the rush of the flow, each set on its own, in rows of three. The first row is the whole stream: **volume**, **tumble** (from an even flow to water arriving in bursts as it tumbles over stones, each easing in and out — it bunches the bubbles, and sways the rush only a little) and **width**. **Bubbles** is how many (a few to a froth), with their **level** and **size** (small, high, glassy bubbles to large, low gurgles). The next row shapes each bubble: **spread** is how much their sizes vary (all alike at the far left, as authored in the middle, far apart at the right), **rise** how far a bubble's pitch climbs as it surfaces (a flat plop to a chirp), and **ring** how long it sounds (a dead plop to a ringing note). The last row is the **rush** beneath them: its texture (a sparse, gravelly rattle to a smooth rush), its **level** (off leaves the bubbles alone) and its **tone**, two octaves either way.
- **Fire channels** are laid out in rows of three. The first row is the whole fire: **volume**, **size** (from embers to a blaze — the weight and depth of the roar) and **width** (from a point at its pan to as wide as the pan allows). The next three rows are its crackle, its pops and its sizzle, each as how much, how loud (**level**: off at the far left, as it is at three quarters, louder above) and **tone**. **Crackle** is how often the wood crackles, and its tone moves the crackles' pitch two octaves either way. **Pops** are sap bursting — dry cracks with no ring — and their tone runs from a dull thud to a bright crack. **Sizzle** is the chance that a pop bursts open a pocket of moisture, which then sizzles on from that moment at the pop's own place — holding with only a gentle waver, then fading out over a few seconds; at most four sizzle at once. Every sizzle is a little different — its own loudness (mostly faint, now and then a loud one), its own place around the tone (its own pitch and frazzle), its own length, and a pitch that drifts as it goes; as it fades it sinks lower and grows more frazzled, as the pocket dries out. Its tone sets both the pitch, from 4 to 8 kHz, and the texture, without changing its loudness: to the left it is frazzled, a crackling fizz of tiny bursts; to the right a smooth hiss. The last row is how the flames themselves move: **flicker** is how far the roar swings (a steady burn to the left, surging and faltering to the right), **period** how long one movement lasts on average, and **dynamics** how abrupt and irregular the movements are — soft swells at an even pace to the left, sudden lurches at an uneven one to the right.
- **Chimes channels** are a set of tubes hanging in a ring around a striker that the wind moves. **Pitch** is the lowest tube, in semitone steps from A440, **tubes** how many there are, **ring** how long a struck tube keeps sounding, **activity** how often the striker is set moving before the wind has any say, and **hardness** runs from a soft striker, warm and round, to a hard one, bright with a tick. **Unison** is how much the chimes sound together: to the left each strike is a single note; to the right the striker rebounds across the ring into a quick cascade of several. **Material** is what the tubes are made of — **wood** (bamboo: a short, hollow knock), **metal** (the classic long ring), **glass** (brighter and shorter) and a **veil** of shimmering tones that swell in rather than strike — with everything between two of them a blend. **Scale** steps through sixty-six tunings, one at a time: the major pentatonic of an ordinary garden chime first, then other pentatonic scales from Japan, China, Ethiopia and Indonesia, just and microtonal tunings, the church modes and other seven-note scales, maqam and raga, and finally stranger ones — equal divisions of the octave into 7, 9, 11, 17, 19, 22 and 31, the harmonic and subharmonic series, and Bohlen-Pierce, which repeats at the twelfth instead of the octave. Chimes follow the weather strongly by default — they are what the wind plays.
...
- Adjusting a channel, the space or the weather makes the plus button active. Press it to save the current soundscape as a numbered button below the factory rows. Hold right-click on it to start from nothing: every channel goes back to its defaults and is turned off, leaving the space and the weather as they are. Select a numbered button to restore it; right-click it, then click it to remove it; hold right-click on it to export it as a \`.tds\` file. Ambient soundscapes are independent of appearance layouts.
...
- Export or import your custom soundscapes as a \`.tds\` file the same way. Importing adds them after your own, skipping any you already have, up to twelve.
