...
- Choose one of twelve soundscapes from the first rows, then choose a channel to show its controls below. There are twenty channels, each marked by the icon of its kind and numbered within it: six **noise** (a wave), three **rain** (a cloud), three **thunder** (a lightning bolt), two **water** (a drop), two **fire** (a flame), two **chimes** (a bell) and two **mark tree** (a wand). Click a channel to view it; hold left-click on a disabled channel to enable it, or hold right-click on an enabled channel to disable it — a disabled channel costs nothing to play. A short right-click solos a channel and shows its controls; right-click it again to clear solo, or right-click another channel to switch solo. The soloed button is highlighted. Disabled channels keep their settings, and their controls remain visible but cannot be changed. While hovering over an enabled channel button, scroll the mouse wheel to adjust its volume in 5% steps.
