...
- **Verbose descriptions** decides how much an effect explains itself. On (the default), "+20% Accuracy (of your misses)" spells out that a chance takes a share of what is left rather than twenty flat points — which is worth knowing once and tedious at every fight after that. Off, the same effect reads "+20% Accuracy": what changed, and nothing else. Nothing is abbreviated beyond recognition either way; only the explanation goes, along with the flavour line under a class move.
