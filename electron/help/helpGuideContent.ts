...
- The bars and rails around the empty editor become the game's own while it runs: **what you are carrying** shows as icons where a note's history timeline sits — items from the left, traits from the right — and hovering one tells you exactly what it is doing for you, with live numbers. Those icons are the one part of the game you press outside the menu: **click one to keep it** when the level ends — one item and one trait, everything else left behind. Whatever is lit is what you keep, and it starts on the newest things you found, so you only have to press anything if you disagree. The **level and stage** read where the word count would be, and the **rail down the right** fills toward your next stat point.
