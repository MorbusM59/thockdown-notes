...
function normalizeUiLayoutLoadout(input: unknown): UiLayoutLoadout | null {
  ...
  return {
    ...
    glazeMode,
    darkMode,
    filterGrayscale: Number.isFinite(source.filterGrayscale) ? Math.max(0, Math.min(1, source.filterGrayscale)) : 0,
    filterInvert: Number.isFinite(source.filterInvert) ? Math.max(0, Math.min(1, source.filterInvert)) : 0,
    filterSepia: Number.isFinite(source.filterSepia) ? Math.max(0, Math.min(1, source.filterSepia)) : 0,
    filterHueRotate: Number.isFinite(source.filterHueRotate) ? Math.max(0, Math.min(360, source.filterHueRotate)) : 0,
    filterBrightness: Number.isFinite(source.filterBrightness) ? Math.max(0, Math.min(2, source.filterBrightness)) : 1,
    filterContrast: Number.isFinite(source.filterContrast) ? Math.max(0, Math.min(2, source.filterContrast)) : 1,
    filterSaturate: Number.isFinite(source.filterSaturate) ? Math.max(0, Math.min(10, source.filterSaturate)) : 1,
    highlightColors: {
