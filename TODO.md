...
- [ ] Retire `resolveIdentityLabel`'s derived-snippet fallback for NOTES (chapters still need it). Now that every note is backfilled with a provisional id at startup, the `isAssigned: false` branch is unreachable for notes -- but it is still the code path that renders a title snippet in italics, which now means the same thing visually as a provisional id. Left in place deliberately: it is harmless while unreachable, and removing it means auditing every `resolveIdentityLabel` call site, which is a bigger job than it looks.
