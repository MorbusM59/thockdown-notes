...
/** The burst envelope of a noise layer, sampled for the worklet. */
...
export type AmbientWorkletChannel = AmbientChannelSettings & {
  ...
  outputIndex: number;
  /** Precomputed burst envelope; noise layers only. */
...
export function toWorkletChannels(settings: AmbientSettings): AmbientWorkletChannel[] {
  return settings.map((channel, index) => (
    channel.kind === 'noise'
      ? { ...channel, outputIndex: -1, envelope: buildAmbientEnvelope(channel.ramp, channel.shape) }
