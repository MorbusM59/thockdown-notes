...
export interface ChaptersApi {
  ...
  /** Detaches `chapterNoteId` from `parentNoteId`'s chapters table (closing the position gap, like removeChapter), remembering its parent + position + chapterId so restoreDetachedChapter can put it back exactly as it was. Tag-agnostic -- the caller applies 'deleted' (Trash) or 'archived' separately, alongside this call. */
  detachChapter(parentNoteId: string, chapterNoteId: string): Promise<ChapterEntry[]>;
  /** Reverses detachChapter: reattaches the chapter to its remembered parent at its remembered (clamped) position and chapterId, shifting that position and everything after it forward to make room. Returns null if the chapter has no remembered detachment (e.g. already restored, or never a chapter). Tag-agnostic, same as detachChapter -- the caller clears 'deleted' or 'archived' separately. */
  restoreDetachedChapter(chapterNoteId: string): Promise<{ parentNoteId: string; chapters: ChapterEntry[] } | null>;
  /** The read-only "virtual merge" that lets an *archived* parent's own chapter bar show every chapter, live or archived-and-detached, without writing anything back to the `chapters` table -- reconstructs the original document order exactly by replaying each archived chapter's own detachment in reverse chronological order (see databaseService.ts's own doc comment for why that's provably correct, unlike a naive position-only splice). Only meaningfully different from listChapters when the parent itself is archived; call listChapters for every other case, since this does needless extra work otherwise. */
  listChaptersIncludingArchived(parentNoteId: string): Promise<ChapterEntry[]>;
