...
- The `App.tsx`/`SectionHandle` structural hazard this round found but didn't fix: `reportSectionHandle`'s shallow diff across *all* of `SectionHandle`'s fields (including `activeNoteText`/`currentEditorText`) means `activeSectionSnapshot` changes identity every keystroke, so *any* future `App.tsx` code keyed on the whole snapshot object is implicitly hot regardless of whether it looks like it should be cold — this round worked around it locally (debouncing the one known consumer) rather than fixing the root cause, which would mean either splitting `SectionHandle` into a hot and cold slice or giving `reportSectionHandle` a narrower comparison. Not attempted — bigger blast radius than this round's scope, and no other consumer of the whole-snapshot object was found to be similarly hot this round, but a future audit should specifically check for this pattern before assuming a new `App.tsx` effect is cold just because it "only runs on note switch."
